...
#define Console_Exit_If_Needed_()	\
	if (key_consoleactive & KEY_CONSOLEACTIVE_USER_1) {	\
		key_consoleactive ^= KEY_CONSOLEACTIVE_USER_1;	\
		key_line[0] = ']';								\
...
static void Host_Kick_f (void)
{
	...
	if (i < svs.maxclients)
	{
		...
		SV_DropClient (false, /*Baker fill this in*/ ""); // kicked
	}
...
void Host_InitCommands (void)
{
	...
	Cmd_AddCommand_WithClientCommand ("prespawn", NULL, SV_PreSpawn_f, "signon 1 (client acknowledges that server information has been received)");
	Cmd_AddCommand_WithClientCommand ("spawn", NULL, SV_Spawn_f, "signon 2 (client has sent player information, and is asking server to send scoreboard rankings)");
	Cmd_AddCommand_WithClientCommand ("begin", NULL, SV_Begin_f, "signon 3 (client asks server to start sending entities, and will go to signon 4 (playing) when the first entity update is received)");
